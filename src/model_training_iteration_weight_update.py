...
class TrainerSingle():
    ...
    def _run_batch(self, queries, explanations, labels):
        ...
        with torch.enable_grad():
            ...
            w_old = {name: param.data.clone() for name, param in self.model.named_parameters()}
            self.scaler.step(self.optimizer)
            w_updated = {name: param.data for name, param in self.model.named_parameters()}
            self.scaler.update()
            alpha = 0.8
            # Manually update weights
            with torch.no_grad():
                for name, param in self.model.named_parameters():
                    param.data = alpha * w_updated[name] + (1 - alpha) * w_old[name]
...
def main_ddp(rank, world_size):
    ...
    if utils.is_main_process():
        ...
        wandb.save(f'advnet/{os.path.basename(__file__)}', policy='now')
        wandb.save('datasets.py', policy='now')
